...
class ROSMonitor:
    def __init__(self):
        ...
        self.pos_broadcast_port  = rospy.get_param("pos_broadcast_port", 65433)
        ...
        self.racecarSocket.bind(('127.0.0.1',self.pos_broadcast_port)) # Bind to address
    ...
    def broadcastFrequency_cb(self, event):
        if not rospy.is_shutdown():
            ...
            self.racecarSocket.sendto(vehiculePosPack,("127.0.0.1",self.pos_broadcast_port)) # Send vehicule position NEED TO FIND ADDRESS!
            print(vehiculePosPack)
        else :
            self.racecarSocket.close()
    ...
    def rr_loop(self):
        ...
        self.rr_socket.bind(('127.0.0.1',self.remote_request_port)) # Bind to address
        ...
        while True:
            ...
            while True:
                userInput = clientConnection.recv(16) # Receive data from the socket (TCP)
                userInputUnpack = unpack(userInputFormat, userInput)[0].decode().strip()# Convert binary to string
                if userInputUnpack == "RPOS":
